...
public class PersonRepositoryTests<K, V> {
    ...
    public void setUp() {
        ...
        // 由于 execute 存在 RedisCallback 和 SessionCallback 两种形式，因此必须显示地确定类型
    ...
    public void saveSingleEntity() {
        ...
        assertThat(
                redisTemplate.execute((RedisConnection connection) -> connection.exists(("persons:" + eddard.getId()).getBytes(CHARSET))),
                is(true));
    }
    ...
    private void flushTestUsers() {
        repository.save(Arrays.asList(eddard));
